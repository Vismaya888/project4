document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add click handler for "Show more" link
    const showMoreLink = document.querySelector('.suggestion-footer a');
    if (showMoreLink) {
    ...
    // Check for new messages every 5 seconds if user is authenticated
...
// Add this function to handle loading more suggestions
function load_more_suggestions() {
    const currentCount = document.querySelectorAll('.suggestion-user').length;
    
    fetch(`/n/load-more-suggestions?offset=${currentCount}`)
        .then(response => response.json())
        .then(suggestions => {
            if (suggestions.length === 0) {
                // If no more suggestions, disable the show more link
                const showMoreLink = document.querySelector('.suggestion-footer a');
                showMoreLink.style.color = '#999';
                showMoreLink.style.pointerEvents = 'none';
                showMoreLink.textContent = 'No more suggestions';
                return;
            }
            ...
            suggestions.forEach((user, index) => {
                ...
                suggestionDiv.className = 'suggestion-user new-suggestion';
                suggestionDiv.innerHTML = `
                    ...
                    <div>
                        <button class="btn btn-outline-success" type="button" onclick="follow_user(this,'${user.username}','suggestion')">Follow</button>
                    </div>
                `;
                
                // Insert before the suggestion footer
                const footer = suggestionBox.querySelector('.suggestion-footer');
                footer.insertAdjacentElement('beforebegin', suggestionDiv);
                
                // Trigger animation after a small delay
                setTimeout(() => {
                    suggestionDiv.style.maxHeight = '100px';
...
function performSearch(query) {
    fetch(`/n/search?q=${encodeURIComponent(query)}`)
        .then(response => response.json())
        .then(results => {
            const searchResults = document.querySelector('.search-results');
    ...
    fetch('/n/messages/unread/total')
        ...
        .then(data => {
            ...
            if (data.count > 0) {
                ...
                // Add animation class if it's a new notification
                if (badge.textContent !== data.count.toString()) {
